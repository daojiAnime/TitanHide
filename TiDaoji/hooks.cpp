...
//https://github.com/mrexodia/TiDaoji/issues/27
    ...
    OUT PULONG ReturnLength OPTIONAL)
{
            ...
            Hider::IsHidden(targetPid, HideNtGetContextThread))
    {
        ...
        Log("[TIDAOJI] NtGetContextThread by %d\r\n", pid);
    ...
    if(NT_SUCCESS(Status) && ThreadInformationClass == ThreadHideFromDebugger)
    {
        ...
        {
            Log("[TIDAOJI] NtQueryInformationThread(ThreadHideFromDebugger) by %d\r\n", pid);
    ...
    IN ULONG ThreadInformationLength)
{
    ...
    if(ThreadInformationClass == ThreadHideFromDebugger && !ThreadInformationLength)
    {
        if(Hider::IsHidden(pid, HideThreadHideFromDebugger))
        {
            Log("[TIDAOJI] NtSetInformationThread(ThreadHideFromDebugger) by %d\r\n", pid);
            PETHREAD Thread;
            ...
            Hider::IsHidden(pid, HideNtSetContextThread))
    {
        ...
        Log("[TIDAOJI] NtSetContextThread by %d\r\n", pid);
    ...
    if(Hider::IsHidden(pid, HideNtClose))
    {
        ...
        if(ObStatus != STATUS_INVALID_HANDLE)  // Don't change the return path for any status except this one
        {
                    ...
                    (HandleInfo.HandleAttributes & OBJ_PROTECT_CLOSE))
            {
                ...
                Log("[TIDAOJI] NtClose(0x%p) (protected handle) by %d\r\n", Handle, pid);
    ...
    OUT PULONG ReturnLength OPTIONAL)
{
    ...
    if(NT_SUCCESS(ret) && SystemInformation)
    {
        ...
        case SystemKernelDebuggerInformation:
        {
            ...
            {
                Log("[TIDAOJI] SystemKernelDebuggerInformation by %d\r\n", pid);
        ...
        case SystemKernelDebuggerInformationEx:
        {
            if (Hider::IsHidden(pid, HideSystemDebuggerInformation))
            {
                Log("[TIDAOJI] SystemKernelDebuggerInformationEx by %d\r\n", pid);
        ...
        case SystemFirmwareTableInformation:
        {
            ...
            __try
            {
                ...
                if(patched)
                {
                    Log("[TIDAOJI] SystemFirmwareTableInformation patched (provider=%.4s) by %d\r\n",
    ...
    OUT PULONG ReturnLength)
{
            ...
            ProcessInformationClass != ProcessBasicInformation) //prevent stack overflow
    {
        if(ProcessInformationClass == ProcessDebugFlags)
        {
            ...
            {
                Log("[TIDAOJI] ProcessDebugFlags by %d\r\n", pid);
                __try
                {
                    BACKUP_RETURNLENGTH();
        ...
        else if(ProcessInformationClass == ProcessDebugPort)
        {
            ...
            {
                Log("[TIDAOJI] ProcessDebugPort by %d\r\n", pid);
                __try
                {
                    BACKUP_RETURNLENGTH();
    ...
    OUT PULONG ReturnLength)
{
    ...
    {
        Log("[TIDAOJI] NtSystemDebugControl by %d\r\n", pid);
    ...
    IN PPS_ATTRIBUTE_LIST AttributeList OPTIONAL)
{
    ...
    if(Hider::IsHidden(pid, HideThreadHideFromDebugger))
    {
        if((CreateFlags & THREAD_CREATE_FLAGS_HIDE_FROM_DEBUGGER) != 0)
        {
            ...
            Log("[TIDAOJI] NtCreateThreadEx with THREAD_CREATE_FLAGS_HIDE_FROM_DEBUGGER by %u\r\n", pid);
        }
    }
