...
NTSTATUS NTDLL::Initialize()
{
    ...
    {
#ifdef _DEBUG
        DbgPrint("[TIDAOJI] KeGetCurrentIrql != PASSIVE_LEVEL!\n");
#endif
    ...
    if(NT_SUCCESS(NtStatus))
    {
        ...
        if(NT_SUCCESS(NtStatus))
        {
            ...
            Log("[TIDAOJI] FileSize of ntdll.dll is %08X!\r\n", StandardInformation.EndOfFile.LowPart);
            ...
            if(!NT_SUCCESS(NtStatus))
            {
                ...
                Log("[TIDAOJI] ZwReadFile failed with status %08X...\r\n", NtStatus);
            }
        }
        else
            Log("[TIDAOJI] ZwQueryInformationFile failed with status %08X...\r\n", NtStatus);
        ZwClose(FileHandle);
    }
    else
        Log("[TIDAOJI] ZwCreateFile failed with status %08X...\r\n", NtStatus);
    return NtStatus;
}
...
int NTDLL::GetExportSsdtIndex(const char* ExportName)
{
    ...
    if(SsdtOffset == -1)
    {
        Log("[TIDAOJI] SSDT Offset for %s not found...\r\n", ExportName);
    }
